...
public class Exo1clientUDPavecbonaffichage {
	public static void main(String[] args) throws Exception
	{
		Exo1clientUDPavecbonaffichage serveurUDP = new Exo1clientUDPavecbonaffichage();
		serveurUDP.execute();
	}
	...
	private void execute() throws IOException
	{
		...
		for(int i =1;i<=10;i++) {
			...
			System.out.println("Le serveur a répondu "+nb1+" et "+nb2);

			dpE = new DatagramPacket(bufE, bufE.length, adrDest); //envoie du message a l'adresse
			socket.send(dpE);
			System.out.println("Envoi d'un paquet UDP avec "+res+";");
